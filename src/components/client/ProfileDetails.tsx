      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {!isEditing ? <>
                  ...
                  <EditableField label="Name" value={formData.name} onChange={value => setFormData(prev => ({
                ...prev,
                name: value as string
              }))} />
                  <EditableField label="Role" value={formData.role} onChange={value => setFormData(prev => ({
                ...prev,
                role: value as string
              }))} />
                  <div className="col-span-2">
                    <EditableField label="Writing Style" value={formData.writingStyle} onChange={value => setFormData(prev => ({
                  ...prev,
                  writingStyle: value as string
                }))} type="textarea" />
                  </div>
                </>}
            </div>
          ...
          <div>
            ...
            {profile.linkedinConnected ? <div className="flex items-center gap-3 p-3 bg-secondary rounded-lg">
              ...
              </div> : <div className="flex items-center justify-center p-6 bg-secondary rounded-lg">
                <Button>
                  <Linkedin className="h-4 w-4 mr-2" />
                  Connect LinkedIn
                </Button>
              </div>}
            {profile.linkedinConnected && <div className="mt-3">
                <span className="block text-xs font-semibold text-gray-500 mb-1">LinkedIn Profile URL</span>
                <a href={brandProfile.linkedinProfileUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline text-sm">
          ...
          <div>
            ...
            {!isEditing ? <p className="text-sm leading-relaxed">{profile.customInstructions}</p> : <EditableField label="" value={formData.customInstructions} onChange={value => setFormData(prev => ({
            ...prev,
            customInstructions: value as string
          }))} type="textarea" placeholder="Enter custom AI instructions for this profile..." />}
          </div>
        </CardContent>
      </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {brandProfile.brandPersonality.map((trait, idx) => <Badge key={idx} variant="outline">{trait}</Badge>)}
              </div>
            </div>
            <div>
            ...
            <div className="col-span-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {brandProfile.emotionsToEvoke.map((emotion, idx) => <Badge key={idx} variant="secondary">{emotion}</Badge>)}
              </div>
            </div>
          </div>
