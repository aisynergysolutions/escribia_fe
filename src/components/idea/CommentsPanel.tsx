          ...
          <TabsContent value="internal" className="flex-1 mt-4">
            <ScrollArea className="h-full pr-3">
              <div className="space-y-4">
                ...
                {activeComments.map(thread => <Card key={thread.id} className="bg-slate-50">
                    <CardHeader className="p-4">
                      <blockquote className="border-l-2 pl-3 italic text-sm text-muted-foreground">"{thread.selectionText}"</blockquote>
                    </CardHeader>
                    <CardContent className="p-4 pt-0 space-y-3">
                      {thread.replies.map(reply => <div key={reply.id}>
                          <p className="text-sm font-semibold">{reply.author} <span className="text-xs text-muted-foreground font-normal">{new Date(reply.createdAt).toLocaleString([], {
                          dateStyle: 'short',
                          timeStyle: 'short'
                        })}</span></p>
                          <p className="text-sm whitespace-pre-wrap">{reply.text}</p>
                        </div>)}
                      {replyingTo === thread.id ? <div className="pt-2">
                        ...
                        </div> : <div className="flex justify-end gap-2 mt-2 border-t pt-3">
