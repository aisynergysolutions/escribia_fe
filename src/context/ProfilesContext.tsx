...
export const getCompanyProfile = async (agencyId: string, clientId: string, profileId: string): Promise<CompanyProfile> => {
  ...
  return {
    ...
    linkedin: {
      ...
      linkedinCompanyConnected: !!data.linkedin?.linkedinConnected,
      linkedinCompanyToken: data.linkedin?.linkedinAccessToken || '',
      linkedinCompanyExpiryDate: data.linkedin?.linkedinExpiryDate || null,
      linkedinCompanyAccountName: data.linkedin?.linkedinAccountName || '',
      linkedinCompanyProfile: data.linkedin?.linkedinOrganizationId ? {
        organization_id: data.linkedin.linkedinOrganizationId || '',
        organization_urn: data.linkedin.linkedinOrganizationUrn || '',
        company_name: data.linkedin.linkedinAccountName || '',
        linkedin_url: data.linkedin.linkedinURL || '',
        connected_scopes: data.linkedin.linkedinScope || '',
        expires_at: data.linkedin.linkedinExpiryDate || '',
      } : undefined,
      linkedinCompanyConnectedAt: data.linkedin?.connectedAt || '',
      linkedinCompanyUpdatedAt: data.linkedin?.lastUpdated || data.linkedin?.updatedAt || '',
    },
    role: data.role || '',
    createdAt: data.createdAt?.seconds ?
      new Date(data.createdAt.seconds * 1000).toISOString() :
